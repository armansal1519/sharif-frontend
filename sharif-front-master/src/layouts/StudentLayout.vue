...
export default {
  ...
  setup() {
    ...
    const { getIsAuth, student, hasSendForm, form, formData, formDataLoaded } =
      storeToRefs(store);
    ...
    onMounted(async () => {
      if (
        localStorage.getItem("s-access") !== null &&
        localStorage.getItem("s-access") !== ""
      ) {
        const resp = await api.get("/student/by-token");
        if (resp.status === 200) {
      ...
      const resp2 = await api.get("form/check");
      if (resp2.status === 200) {
        console.log(resp2.data);
        hasSendForm.value = resp2.data.formExist;
        form.value = resp2.data.form;
      }

      const resp3 = await api.get("form/student");
      if (resp3.status === 200) {
        console.log(resp3.data);
        formData.value = resp3.data;
      }
